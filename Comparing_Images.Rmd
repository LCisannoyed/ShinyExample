...
thresholds<-reactive({ 
  ...
  # get first case of archive data as a temp measure
   chart_directory <- "Archived_data/"
   temp_archived_data<-list.files(chart_directory, 
                                      "*.RData", full.names = F)[1]
    temp_archived_data <- paste('Archived_data/',temp_archived_data, sep="")
 

  # real archived data to match selection  criteria
  timestamp <-substr(selected_fname(), regexpr('\\(', selected_fname())+1, regexpr('\\)', selected_fname())-1) 
  ...
  tryCatch(
      ...
      error=function(cond) {
       # Choose a return value in case of error
       data_frame(round=all_rounds,itm_params=all_rounds )   
        },
     warning=function(cond) {
         # Choose a return value in case of error
       data_frame(round=all_rounds,itm_params=all_rounds )   
